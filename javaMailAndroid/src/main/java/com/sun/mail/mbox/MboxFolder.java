...
e.printStackTrace();
	...
	try {
		...
		new FileOutputStream(folder.getPath(), true));
		// XXX - should use getAbsolutePath()?
        for (Message msg : msgs) {
            if (msg instanceof MimeMessage) {
                writeMboxMessage((MimeMessage) msg, os);
            } else {
                err = true;
                continue;
            }
