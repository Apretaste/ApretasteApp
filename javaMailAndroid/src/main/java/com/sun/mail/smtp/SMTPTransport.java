...
public class SMTPTransport extends Transport {
	...
	// it has all valid InternetAddresses; fail if not
		...
		for (Address address : addresses) {
			if (!(address instanceof InternetAddress)) {
				throw new MessagingException(address +
	...
	for (int i = 0; i < addresses.length; i++) {
		...
		try {
		    ...
		    if (ia != null) {
				for (InternetAddress anIa : ia) groups.addElement(anIa);
		    } else
			groups.addElement(a);
	...
	if (debug) {
		...
		out.println("DEBUG SMTP: Verified Addresses");
			for (Address aValidSentAddr : validSentAddr) {
				out.println("DEBUG SMTP:   " + aValidSentAddr);
			}
	    }
	    if (validUnsentAddr != null && validUnsentAddr.length > 0) {
		out.println("DEBUG SMTP: Valid Unsent Addresses");
			for (Address aValidUnsentAddr : validUnsentAddr) {
				out.println("DEBUG SMTP:   " + aValidUnsentAddr);
			}
	    }
	    if (invalidAddr != null && invalidAddr.length > 0) {
		out.println("DEBUG SMTP: Invalid Addresses");
			for (Address anInvalidAddr : invalidAddr) {
				out.println("DEBUG SMTP:   " + anInvalidAddr);
			}
	    }
	}
