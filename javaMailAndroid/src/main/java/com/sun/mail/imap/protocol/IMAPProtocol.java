...
public class IMAPProtocol extends Protocol {
	...
	authmechs = new ArrayList(5);
        for (Response aR : r) {
            if (!(aR instanceof IMAPResponse))
                continue;

            IMAPResponse ir = (IMAPResponse) aR;
	...
	if (allowed != null && allowed.length > 0) {
        ...
        for (String anAllowed : allowed)
            if (authmechs.contains(anAllowed))    // XXX - case must match
                v.add(anAllowed);
	} else {
	...
	if (items == null)
        ...
        for (String item : items) itemArgs.writeAtom(item);
	...
	Response[] r = fetch(String.valueOf(uid), "UID", true);
        ...
        for (Response aR : r) {
            if (aR == null || !(aR instanceof FetchResponse))
                continue;

            FetchResponse fr = (FetchResponse) aR;
