...
public class IMAPMessage extends MimeMessage {
	...
	Utility.Condition condition = new FetchProfileCondition(fp);
		...
		return;
            ...
            for (Response aR : r) {
                if (aR == null)
                    continue;
                if (!(aR instanceof FetchResponse)) {
                    v.addElement(aR); // Unsolicited Non-FETCH response
                    continue;
                }

                // Got a FetchResponse.
                FetchResponse f = (FetchResponse) aR;
			     ...
			     */
                        ...
                        if (allHeaders)
                            ...
                            for (String hdr : hdrs) msg.setHeaderLoaded(hdr);
                        }
                    }
                }
	...
	synchronized(getMessageCacheLock()) {
		...
		r = p.fetch(seqnum, EnvelopeCmd);
            ...
            for (Response aR : r) {
                ...
                if (aR == null ||
                        !(aR instanceof FetchResponse) ||
                        ((FetchResponse) aR).getNumber() != seqnum)
                    continue;
    ...
    private synchronized boolean isHeaderLoaded(String name) {
		// All headers for this message have been loaded
		return headersLoaded || ((loadedHeaders != null) && loadedHeaders.containsKey(name.toUpperCase(Locale.ENGLISH)));
