...
public class IMAPFolder extends Folder implements UIDFolder, ResponseHandler {
	...
	*/
	...
	if (recurse) {
	    ...
	    Folder[] f = list();
		for (Folder aF : f) aF.delete(recurse); // ignore intermediate failures
	}
	...
	try {
		...
		if (unavailUids.length > 0) {
		    ...
		    IMAPMessage m;
			for (UID anUa : ua) {
				m = getMessageBySeqNumber(anUa.seqnum);
				m.setUID(anUa.uid);
				uidTable.put(anUa.uid, m);
			}
		}
		...
		for (int i = 0; i < uids.length; i++)
		    msgs[i] = (Message)uidTable.get(uids[i]);
		return msgs;
	    }
	} catch(ConnectionException cex) {
	    throw new FolderClosedException(this, cex.getMessage());
