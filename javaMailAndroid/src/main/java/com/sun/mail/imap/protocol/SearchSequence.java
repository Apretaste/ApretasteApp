...
class SearchSequence {
	...
	else if (term instanceof UidTerm) // MessageID
		return uid((UidTerm) term, charset);
	else
	...
	if (term instanceof AndTerm || term instanceof OrTerm) {
        ...
        for (SearchTerm term1 : terms)
            if (!isAscii(term1)) // outta here !
                return false;
	...
	private static Argument uid(UidTerm term, String charset)
			throws SearchException, IOException {
		Argument result = new Argument();
		result.writeAtom("UID");
		if(term.getSingleUid()!=null)
		{
			result.writeNumber(term.getSingleUid());
		}
		else
		{
			result.writeAtom(UIDSet.toString(new UIDSet[]{term.getmUids()}));
		}
		return result;
	}
