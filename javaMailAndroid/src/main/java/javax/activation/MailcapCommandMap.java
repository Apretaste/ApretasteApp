...
public class MailcapCommandMap extends CommandMap {
	...
	if (mimeType != null)
        ...
        for (MailcapFile aDB1 : DB) {
            if (aDB1 == null)
                continue;
            Map cmdMap = aDB1.getMailcapList(mimeType);
            if (cmdMap != null)
                appendPrefCmdsToList(cmdMap, cmdList);
        }

	// now add the fallback commands
        for (MailcapFile aDB : DB) {
            if (aDB == null)
                continue;
            Map cmdMap = aDB.getMailcapFallbackList(mimeType);
            if (cmdMap != null)
                appendPrefCmdsToList(cmdMap, cmdList);
        }
    ...
    private void appendPrefCmdsToList(Map cmdHash, List cmdList) {
        ...
        for (Object o : cmdHash.keySet()) {
            String verb = (String) o;
    ...
    private boolean checkForVerb(List cmdList, String verb) {
        for (Object aCmdList : cmdList) {
            String enum_verb =
                    ((CommandInfo) aCmdList).getCommandName();
    ...
    private void appendCmdsToList(Map typeHash, List cmdList) {
        ...
        for (Object o : typeHash.keySet()) {
            String verb = (String) o;
            List cmdList2 = (List) typeHash.get(verb);

            for (Object aCmdList2 : cmdList2) {
                String cmd = (String) aCmdList2;
	...
	if (mimeType != null)
        ...
        for (MailcapFile aDB : DB) {
            if (aDB == null)
                continue;
            String[] cmds = aDB.getNativeCommands(mimeType);
            if (cmds != null) {
                for (String cmd : cmds) {
                    // eliminate duplicates
