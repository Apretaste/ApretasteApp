...
public abstract class Transport extends Service {
	...
	Vector validUnsent = new Vector();
        ...
        for (Address address : addresses) {
            ...
            if (protocols.containsKey(address.getType())) {
                Vector v = (Vector) protocols.get(address.getType());
                v.addElement(address);
            } else {
                // need to add a new protocol
                Vector w = new Vector();
                w.addElement(address);
                protocols.put(address.getType(), w);
            }
        }
	...
	while (e.hasMoreElements()) {
		...
		// Mark these addresses invalid.
            for (Address protaddress : protaddresses) invalid.addElement(protaddress);
		continue;
	    }
	    try {
		transport.connect();
		...
		if (a != null)
            for (Address anA : a) invalid.addElement(anA);

		// retrieve validSent addresses
		a = sex.getValidSentAddresses();
		if (a != null)
            for (Address anA : a) validSent.addElement(anA);

		// retrieve validUnsent addresses
		Address[] c = sex.getValidUnsentAddresses();
		if (c != null)
            for (Address aC : c) validUnsent.addElement(aC);
